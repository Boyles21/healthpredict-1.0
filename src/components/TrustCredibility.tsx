...
export default function TrustCredibility() {
  return (
    <section id="trust" className="py-20 bg-white border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-slate-650 font-medium mt-4 text-sm sm:text-base leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {trustFactors.map((factor, index) => (
            <motion.div
              ...
              className="relative p-6 bg-white border border-slate-200 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-teal-500/25 transition-all duration-300 group overflow-hidden flex flex-col text-left"
            >
              ...
              <div className={`w-11 h-11 rounded-lg flex items-center justify-center mb-5 ${factor.iconColor} font-bold z-10 transition-transform group-hover:scale-110 duration-300`}>
                <factor.icon className="w-5.5 h-5.5" />
              </div>

              <h4 className="text-base font-bold text-slate-900 mb-2 relative z-10 group-hover:text-teal-700 transition-colors">
                {factor.title}
              </h4>
              
              <p className="text-sm text-slate-650 leading-relaxed relative z-10 flex-grow">
                {factor.description}
              </p>
            </motion.div>
          ))}
